...
public class Update extends AbstractUpdater<Update, Where, Column> implements FeaturedSetter<Update>, FeaturedWhere<Update> {
    ...
    public String stringfy(String type) {
        ...
        stringBuffer.append(getTable().getTableName());
        ...
        sets.stream().forEach(column -> {
            /*是自动计算字段则不做预编译处理**/
            if(column.getValue() instanceof AutoCalculate){
                AutoCalculate autoCalculate  = ((AutoCalculate) column.getValue());
                stringBuffer.append(column.getName() + " = " + column.getName() + autoCalculate.getSign() + autoCalculate.getValue());
            }else {
